<x-layouts.app :title="__('Add Customer')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">Add New Customer</h1>
                <p class="text-sm text-zinc-600 dark:text-zinc-400">Create a new customer account for IoT device management</p>
            </div>
        ...
        <div class="flex-1 overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800">
            <form method="POST" action="{{ route('customers.store') }}" enctype="multipart/form-data" class="p-6">
                @csrf
                ...
                <div class="mt-8 p-4 bg-zinc-50 dark:bg-zinc-700/50 rounded-lg">
                    ...
                    <div class="text-sm text-zinc-600 dark:text-zinc-400 space-y-1">
                        <p>• Email will be automatically verified upon creation</p>
                        <p>• Customer will have access to mobile app with these credentials</p>
                        <p>• Customer role will be automatically assigned</p>
                    </div>
                </div>

                <!-- Form Actions -->
                <div class="mt-8 flex items-center justify-end gap-3">
                    <flux:button variant="outline" type="button" :href="route('customers.index')">
                        Cancel
                    </flux:button>
    ...
    <script>
        ...
        fetch('/data/india-states-cities.json')
            ...
            .then(data => {
                ...
                // Set old values if form has errors
                @if(old('state'))
                    document.getElementById('state').value = '{{ old('state') }}';
                    populateCities('{{ old('state') }}');
                    @if(old('city'))
                        setTimeout(() => {
